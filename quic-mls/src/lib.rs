...
#[test]
    ...
    fn next_1rtt_keys_reflects_new_epoch_after_commit() {
        ...
        //alice sending encrypts the payload in buf1, leaving the header untouched. The tag is written to the end of buf1.
        alice_epoch1.local.encrypt(0, &mut buf1, header_len);
        //cipher text copy
        let epoch1_ciphertext = buf1[header_len..].to_vec();
        //bob receiving decrypts the payload in place, leaving the header untouched. The tag is expected to be at the end of the payload.
