...
DURATION=90
REPORT_TIMEOUT=8
TRANSCRIPT_MAX_BYTES=900   # default; kept fixed across the sweep deliberately

echo "=== 0. Sanity check the transcript mechanism is present ==="
grep -q "encode_transcript" testbed-runner/src/main.rs \
...
for INTERVAL in "${COMMIT_INTERVALS[@]}"; do
    ...
    sudo pkill -f "testbed-runner --role bob" 2>/dev/null || true
    sleep 1

    sudo ip netns exec ns-bob ./target/release/testbed-runner \
    ...
    sudo ip netns exec ns-alice ./target/release/testbed-runner \
